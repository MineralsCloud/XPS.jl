...
!!! info
    The style guide is always a work in progress, and not all ExpressCommands code
    follows the rules. When modifying ExpressCommands, please fix the style violations
...
`ExpressCommands` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/ExpressCommands.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `ExpressCommands` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `ExpressCommands` have
